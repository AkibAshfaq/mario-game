...
function spawnParticles(cx: number, cy: number): void {
...
function randomLane(excludeLane?: number): number {
  const lanes = [0, 1, 2].filter(l => l !== excludeLane);
  return lanes[Math.floor(Math.random() * lanes.length)];
}

function scoreMeters(): number {
...
function drawRoad(dark: boolean): void {
...
function showIdle(): void {
  ...
  (document.querySelector('.score-big') as HTMLElement | null)?.remove?.();
  (document.querySelector('.best-line') as HTMLElement | null)?.remove?.();
